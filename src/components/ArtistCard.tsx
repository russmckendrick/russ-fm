...
export function ArtistCard({ artist, onClick }: ArtistCardProps) {
  ...
  const CardWrapper = ({ children }: { children: React.ReactNode }) => {
    ...
    return (
      <Link to={artist.uri} className="block h-full">
        {children}
      </Link>
    );
  };
      ...
      <CardWrapper>
        <Card className="h-full border-0 bg-transparent shadow-none overflow-visible">
          <CardContent className="p-0 h-full flex flex-col gap-4">
            ...
            <div className="relative aspect-square rounded-full overflow-hidden shadow-md transition-all duration-500 group-hover:shadow-xl group-hover:scale-[1.02] border-2 border-transparent group-hover:border-white/10">
              <img
                src={artist.image}
                alt={artist.name}

...
class ImageManager:
    ...
    def __init__(self, base_path: str = "data/releases", config: Dict[str, Any] = None):
        ...
        if config and "image_sizes" in config:
            ...
            image_sizes = {}
            for size_name, size_str in config["image_sizes"].items():
                if "x" in size_str:
            ...
            self.image_sizes = {"hi-res": 2000, "medium": 800, "small": 300}
    ...
    def download_album_artwork(
        ...
        for size_name, size_pixels in self.image_sizes.items():
            ...
            if self.download_image(sized_url, file_path):
                ...
                logger.info(f"Downloaded {size_name} artwork for {release_title}")
            else:
                downloaded_images[size_name] = None
                logger.warning(
                    f"Failed to download {size_name} artwork for {release_title}"
                )
        ...
        for size_name, size_pixels in self.image_sizes.items():
            ...
            for source in image_sources:
                ...
                if self.download_image(sized_url, file_path, user_agent=user_agent):
                    ...
                    if source_type == "spotify":
                        logger.info(
                            f"Downloaded {size_name} artwork for {release_title} from {source_type} ({spotify_size})"
                        )
                    else:
                        logger.info(
                            f"Downloaded {size_name} artwork for {release_title} from {source_type}"
                        )

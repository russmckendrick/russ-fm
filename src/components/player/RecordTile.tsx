 ...
 * out on hover), a colour bar in the sleeve's colours (flood, plus the
 * secondary colour when there is one) and the title.
 */
export function RecordTile({ album, palette, meta, showArtist = true, showText = true, className, to }: RecordTileProps) {
  ...
  return (
    <Link to={to ?? album.uri_release} className={cn('rec group block min-w-0', className)}>
      ...
      <div className="mt-2.5 h-1 w-full" style={{ background: colourBar(colours) }} aria-hidden />
      {showText && (

...
The **Boxsets** home-menu entry lists every box-set release in the database (any `formats[]`
entry containing "box" — the same rule `collection.json` uses for `format_primary: "Box Set"`)
split across two views, toggled with `Tab`:

- **Unprocessed** — boxes with no linked member albums yet. `Enter` runs **boxset discovery**
  for the selected box in the background: the same workflow as
  ...
  (the CLI's `--force-refresh`). Boxes whose Discogs tracklist has no album section headers are
  flagged `· no headers`; discovery on one of those refetches the box (which heals old rows with
  stripped headers) and, if it is still headerless, logs the "link members individually" error
  — link those with `scrapper release <ALBUM_ID> --save --boxset <BOX_ID>`.
...
ID — which is why the run actions take `Ctrl`). A progress gauge and log sit under the list so a
run can be watched in place; `Esc` leaves the screen while the run continues, and any picker it
raises still appears. Only one interactive run (collection, artist batch, detail refresh or
boxset discovery) can be active at a time, because they share the single modal picker. When a
run finishes the list reloads (the box moves to Processed) and, since discovery saves,
